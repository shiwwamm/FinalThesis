...
class ResilienceEnv(gym.Env):
    ...
    Reward types:
      ...
      - 'mult'     : Multiplicative Resilience (λ₂ × GCC × (1-IVI))
      - 'pareto'   : Pareto Dominance (improve ≥1 metric, degrade none)
      - 'spectral' : Spectral Gap Maximization (maximize λ₂ - λ₁)
      - 'resist'   : Effective Resistance Reduction
      - 'attack'   : Direct Attack Resilience Measurement
    """

    metadata = {"render_modes": []}

    def __init__(self, path, reward_type="attack", budget_edges=None, gamma=5.0):
        super().__init__()
        g = ig.Graph.Read_GraphML(path).as_undirected()
        self.g_orig = g.copy()
        self.n = g.vcount()
        self.m = g.ecount()
        assert reward_type in ["ivi", "nnsi", "mult", "pareto", "spectral", "resist", "attack"]
    ...
    def reset(self, seed=None, options=None):
        ...
        # Additional baselines for new reward functions
        # For MULT and PARETO: GCC after attack
        k = max(1, int(0.05 * self.n))
        degrees = np.array(self.g.degree())
        top_k = np.argsort(-degrees)[:k]
        g_attacked = self.g.copy()
        g_attacked.delete_vertices(top_k)
        try:
            self.prev_gcc = g_attacked.clusters().giant().vcount() / (self.n - k)
        except:
            self.prev_gcc = 0.0
        
        # For SPECTRAL: spectral gap
        L = np.array(self.g.laplacian(normalized=True))
        eigvals = np.linalg.eigvalsh(L)
        self.prev_spectral_gap = eigvals[1] - eigvals[0]
        self.prev_lambda_n = eigvals[-1]
        
        # For RESIST: effective resistance
        self.prev_resistance = self.n * np.sum(1.0 / (eigvals[1:] + 1e-10))
        
        # For ATTACK: resilience score
        self.prev_resilience = 0.5  # Initial estimate
    ...
    def _compute_reward(self):
        ...
        if self.reward_type == "ivi":
            ...
            r = d_nnsi
        elif self.reward_type == "mult":
            r = self._reward_mult(curr_lambda2, curr_max_ivi, d_lambda2, d_ivi)
        elif self.reward_type == "pareto":
            r = self._reward_pareto(curr_lambda2, curr_mincut, curr_max_ivi, d_lambda2, d_ivi)
        elif self.reward_type == "spectral":
            r = self._reward_spectral()
        elif self.reward_type == "resist":
            r = self._reward_resist(curr_mincut)
        elif self.reward_type == "attack":
            r = self._reward_attack()
        else:
            r = 0.0
    ...
    def _reward_mult(self, curr_lambda2, curr_max_ivi, d_lambda2, d_ivi):
        """Multiplicative Resilience: r = (1 + Δλ₂) × (1 + ΔGCC) × (1 + ΔIVI)"""
        # GCC after attack
        k = max(1, int(0.05 * self.n))
        degrees = np.array(self.g.degree())
        top_k = np.argsort(-degrees)[:k]
        ...
        g_attacked.delete_vertices(top_k)
        try:
            curr_gcc = g_attacked.clusters().giant().vcount() / (self.n - k)
        except:
            curr_gcc = 0.0
        
        d_gcc = curr_gcc - self.prev_gcc
        
        # Multiplicative reward
        r = (1 + d_lambda2) * (1 + d_gcc) * (1 + d_ivi) - 1
        
        self.prev_gcc = curr_gcc
        return r
    
    def _reward_pareto(self, curr_lambda2, curr_mincut, curr_max_ivi, d_lambda2, d_ivi):
        """Pareto Dominance: Reward if improve ≥1 metric without degrading others"""
        # GCC after attack
        k = max(1, int(0.05 * self.n))
        degrees = np.array(self.g.degree())
        top_k = np.argsort(-degrees)[:k]
        ...
        changes = {
            'lambda2': d_lambda2,
            'mincut': curr_mincut - self.prev_mincut,
            'gcc': curr_gcc - self.prev_gcc,
            'ivi': d_ivi,
        }
        
        # Count improvements and degradations
        improvements = sum(1 for v in changes.values() if v > 1e-6)
        degradations = sum(1 for v in changes.values() if v < -1e-6)
        
        # Pareto-based reward
        if degradations == 0 and improvements > 0:
            r = improvements / len(changes)
        elif improvements > degradations:
            r = (improvements - degradations) / len(changes) * 0.5
        else:
            r = -(degradations / len(changes))
        
        self.prev_gcc = curr_gcc
        return r
    
    def _reward_spectral(self):
        """Spectral Gap Maximization: r = Δ(λ₂ - λ₁)"""
        L = np.array(self.g.laplacian(normalized=True))
        eigvals = np.linalg.eigvalsh(L)
        
        curr_spectral_gap = eigvals[1] - eigvals[0]
        curr_lambda_n = eigvals[-1]
        
        d_spectral_gap = curr_spectral_gap - self.prev_spectral_gap
        d_lambda_n = curr_lambda_n - self.prev_lambda_n
        
        r = d_spectral_gap - 0.1 * d_lambda_n
        
        self.prev_spectral_gap = curr_spectral_gap
        self.prev_lambda_n = curr_lambda_n
        return r
    
    def _reward_resist(self, curr_mincut):
        """Effective Resistance Reduction: Minimize total resistance"""
        L = np.array(self.g.laplacian(normalized=True))
        eigvals = np.linalg.eigvalsh(L)
        
        curr_resistance = self.n * np.sum(1.0 / (eigvals[1:] + 1e-10))
        
        d_resistance = self.prev_resistance - curr_resistance
        d_mincut = curr_mincut - self.prev_mincut
        
        r = d_resistance + 0.5 * d_mincut
        
        self.prev_resistance = curr_resistance
        return r
    
    def _reward_attack(self):
        """Direct Attack Resilience: Simulate attack and measure degradation"""
        k = max(1, int(0.05 * self.n))
        degrees = np.array(self.g.degree())
        top_k_nodes = np.argsort(-degrees)[:k]
        
        g_attacked = self.g.copy()
        g_attacked.delete_vertices(top_k_nodes)
        
        try:
            gcc_size = g_attacked.clusters().giant().vcount()
            gcc_fraction = gcc_size / (self.n - k)
            
            if g_attacked.is_connected():
                aspl = g_attacked.average_path_length()
            else:
                aspl = 10.0
            
            num_components = len(g_attacked.clusters())
            edges_remaining = g_attacked.ecount()
            edge_fraction = edges_remaining / max(1, self.g.ecount())
        except:
            gcc_fraction = 0.0
            aspl = 10.0
            num_components = self.n - k
            edge_fraction = 0.0
        
        curr_resilience = (
            0.5 * gcc_fraction +
            0.2 * edge_fraction +
            0.2 * (1 - min(aspl, 10) / 10) +
            0.1 * (1 - num_components / (self.n - k))
        )
        
        d_resilience = curr_resilience - self.prev_resilience
        
        self.prev_resilience = curr_resilience
        return d_resilience
